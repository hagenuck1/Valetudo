        ...
        this.vacuum.getConsumableStatus((err, res) => {
            if (!err) {
                ...
                this.vacuum.getCleanSummary((err, res2) => {
                    if (!err) {
                        response.totalCleanTime = (res2[0] / 60 / 60).toFixed(1);
                        response.totalCleanArea = (res2[1] / 1000000).toFixed(1);
                        this.vacuum.getCurrentStatus((err3, res3) => {
                            if (!err) {
                                response.currentCleanArea = ("00" + (res3.clean_area / 1000000).toFixed(2)).slice(-6) + " m²";
                                response.currentCleanTime = window.fn.secondsToHms(res3.clean_time);
                            } else {
                                response.currentCleanArea = "00 m²";
                                response.currentCleanTime = "00";
                            }
                        }
...
MqttClient.prototype.handleCustomCommand = function (message) {
    ...
    if (msg && msg.command) {
        switch (msg.command) {
             ...
             * }
             */    
            case CUSTOM_COMMANDS.CHANGE_SOUND_VOLUME:
                if (msg.volume) {
                    if (msg.volume <= 100 && msg.volume >= 0) {
                        this.vacuum.setSoundVolume(msg.volume, () => {
                        });
                    } else {
                        console.info("Invalid sound volume");
                    }
                } else {
                    console.info("Missing sound volume");
                }
                break;
            default:
